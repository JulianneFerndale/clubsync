...
@section('title', 'Edit Officer Record')
@section('club-name', $club?->name ?? 'ClubSync')
@section('page-title', 'Edit Officer Record')
...
<div class="flex items-center gap-3 bg-[#1B5E20] px-5 py-4">
    ...
    <h1 class="text-white font-bold text-xl">Edit Officer Record</h1>
</div>
...
<div class="px-4 py-5">
    ...
    <form method="POST" action="{{ route('clubs.officers.update', $record) }}" class="space-y-4">
        @csrf
        @method('PATCH')
